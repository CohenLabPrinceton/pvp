...
class Controller:
	...
	# Control scheme assumes the use of a PEEP valve
		...
		if(state==0):
			...
			if(jp.get_pressure(1) > self.settings.pip):
				self.__hold_pip()
		elif(state==1):
			...
			if(jp.get_pressure(1) < self.settings.peep): 
				self.__hold_peep()
		elif(state==3):
			# AT REST 
	...
	def __print_status(self):
		...
		print("STATE: %2d pressure_0: %3.1f pressure_1: %3.1f flow: %3.0f"%(self.state,self.jp.get_pressure(1),self.jp.get_pressure(1),self.jp.get_flow()),end="\r")

	def __print_breath(self):
		# TODO -------------------------------------------------
